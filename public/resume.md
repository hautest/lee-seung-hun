...
- 재현이 어려운 상용 SSR 서버의 메모리 누수를 AI 에이전트 기반 진단으로 규명하고 해결하였습니다.
  ...
  - 누수가 상용 환경 조건에서만 발생하여, 힙 스냅샷을 주기적으로 수집해 S3로 전송하는 계측을 상용에 배포하였습니다. 메모리 여유분 기준 가드와 환경변수 스위치를 두어 미설정 시 기존 동작과 완전히 동일하도록 설계하였습니다.
...
- 국내 웹의 Next.js 14 → 16 업그레이드와 Turbopack 도입, React 19 전환을 진행하였습니다.
  ...
  - 프로덕션 빌드에서만 Safari FairPlay 재생이 실패하던 문제(shaka 6010)를 minify 온·오프 이진 분리로 좁혀, 이미 압축된 라이브러리를 번들러가 재압축하며 EME 디스패치가 깨지는 것이 원인임을 확인하였습니다. 해당 라이브러리를 번들에서 분리해 self-host 방식으로 전환하여 해결하고, 부수적으로 전체 청크에서 해당 라이브러리를 제거하였습니다.
