...
- 국내 웹의 Next.js 14 → 16 업그레이드와 Turbopack 도입, React 19 전환을 진행하였습니다.
  ...
  - 프로덕션 빌드에서만 Safari FairPlay 재생이 실패하던 문제(shaka 6010)의 원인 후보를 하나씩 소거해, 이미 압축된 라이브러리를 번들러가 재압축하는 것이 원인임을 확인하였습니다. 해당 라이브러리를 번들에서 분리해 self-host 방식으로 전환하여 해결하고, 부수적으로 전체 청크에서 해당 라이브러리를 제거하였습니다.
